...
Renders the classic GitHub contribution graph in an animated SVG (contrib-heatmap.svg).
- Reads data/contributions.json
- 53 weeks x 7 days grid with rounded boxes (rx="2.5")
- Official dark theme palette: ["#161b22", "#0e4429", "#006d32", "#26a641", "#39d353", "#69f0a0"]
- Diagonal entrance wave animation (runs once on load, freezes)
- English labels, legend "Less -> More" and real yearly contribution statistics
- Exact width of 860px to align with the 370px + 490px = 860px whoami section
"""
...
def render_heatmap(data: dict, output_path: str = "contrib-heatmap.svg"):
    ...
    # 53 columns (weeks). End of grid is Saturday of the last week
    ...
    # Month label positioning without collisions
    month_labels = []
    last_month_col = -10
    ...
    # Diagonal wave animation timing
    base_delay = 0.10
    diag_factor = 0.015
    anim_dur = 0.32
    ...
    for col in range(53):
        ...
        # Check if month changed and avoid overlapping labels
        m_idx = col_start_date.month
        if (col - last_month_col) >= 3 and col < 51:
            # Check if this week is the start of a new month
            if col == 0 or col_start_date.day <= 7:
                last_month_col = col
    ...
    # Legend "Less -> More"
    legend_svg = []
    leg_x = svg_w - 48 - len(PALETTE) * 14 - 70
    leg_y = grid_top + 7 * row_step + 24

    legend_svg.append(f'<text x="{leg_x - 6}" y="{leg_y + 9}" text-anchor="end" class="footer-label">Less</text>')
    ...
    legend_svg.append(f'<text x="{leg_x + len(PALETTE) * 14 + 6}" y="{leg_y + 9}" class="footer-label">More</text>')

    # Footer Statistics (English & valid XML)
...
{"".join(cells_svg)}
  ...
  <text x="{grid_left}" y="{leg_y + 9}" class="footer-stat">
    <tspan class="footer-stat-hl">{formatted_total}</tspan> contributions in the last year • Current Streak: <tspan class="footer-stat-hl">{current_streak} days</tspan> • Longest Streak: <tspan class="footer-stat-hl">{longest_streak} days</tspan>
  </text>
  ...
  {"".join(legend_svg)}
    ...
    print(f"[+] Heatmap SVG generated successfully: {output_path} ({svg_w}x{svg_h})")
