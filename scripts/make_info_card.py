...
Key design decisions for GitHub compatibility:
- All text uses inline fill="..." attributes (not just CSS classes) because
  GitHub's SVG proxy can be unpredictable with CSS-only styling.
- SMIL animations are used alongside CSS keyframes for dual-engine support.
- No external fonts or resources; system font stack only.
- STATIC=1 env var produces a frozen frame (no animation).
"""
...
def generate_info_card_svg(output_path: str = "info-card.svg", is_static: bool = False):
    ...
    data_items = [
        ...
        ("Stack",    "Next.js, TypeScript, React, Supabase, PostgreSQL, Python", "#a371f7"),
        ("Prev",     "Loss Prevention (Magalog) | Cargo Logistics (GFL)",        "#f0883e"),
    ...
    # ── Build CSS (using an f-string so {{ }} becomes { }) ───────────
    anim_css = ""
    if not is_static:
        anim_css = """
      @keyframes slideIn {
...
"""
    ...
    css_block = f"""
      text {{ font-family: {FONT_MONO}; font-size: 12.2px; }}
      {anim_css}
    """
    ...
    def anim_attrs(delay: float) -> str:
        """Return SMIL animate elements and CSS class/style for a row."""
        if is_static:
            return "", "", ""
        cls = ' class="anim"'
        style = f' style="animation: slideIn {duration}s ease {delay:.2f}s forwards;"'
        smil = (
    ...
    for idx, (key, val, col) in enumerate(data_items):
        ...
        if key == "title":
            body.append(f"""    <g{cls}{sty}>{smil}
      <text x="24" y="{cur_y}" font-weight="700" font-size="13.5px" fill="{col}">{escaped}</text>
    </g>""")
            cur_y += 18

        elif key == "sep":
            body.append(f"""    <g{cls}{sty}>{smil}
      <text x="24" y="{cur_y}" fill="{col}">{escaped}</text>
    </g>""")
            cur_y += 20
